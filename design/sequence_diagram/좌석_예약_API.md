...
sequenceDiagram
    ...
    c->> se : Request ( userId, tokenId, SeatId ) : 좌석 예약 요청 
    activate se

    se->>qu : isAvailableToken( userId, tokenId ): 유효한 토큰인지 검증  


    alt 유효한 토큰

        critical TRANSACTIONAL
            se->>se : findSeatByIdWithLock ( seatId ) : 예약할 좌석 불러 오기 

            opt 점유된 좌석인 경우 
                se-->>c : Error Response ("This Seat is already Reserved")
            end 

            se->>se : updateState : "occupied" 로 상태 변환
